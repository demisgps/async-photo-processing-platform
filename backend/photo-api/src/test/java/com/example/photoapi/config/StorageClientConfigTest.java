...
class StorageClientConfigTest {
    ...
    @Test
    void localClientPreservesRetryDeadlines() {
        var client = configuration.storage(properties(URI.create("http://localhost:4443")));
    ...
    private StorageProperties properties(URI endpoint) {
        return new StorageProperties(endpoint, "original", "processed", "configured-project",
