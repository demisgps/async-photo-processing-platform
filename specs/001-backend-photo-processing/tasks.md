...
- [X] T067 [US2] Implementar adapter HTTP Push de `photo-consumer-sub` que responde 204 para commit/no-op válido duplicado/atrasado/fora de ordem/terminal, mas usa resposta diferente de ACK para payload malformado ou contratualmente inválido seguir redelivery finita e DLT em `backend/photo-consumer/src/main/java/com/example/photoconsumer/processamento/consumer/PubSubPushController.java` e `PhotoProcessingMessageHandler.java`
...
- [X] T088 [US6] Endurecer a política básica do adapter Push para idempotência, redelivery e estados terminais: HTTP 204 para commit/no-op de mensagem válida duplicada/atrasada/fora de ordem/terminal/logicamente não aplicável, inclusive `PhotoProcessingError` em `PERSISTINDO`; resposta diferente de ACK para falha transitória ou mensagem malformada/contratualmente inválida em `backend/photo-consumer/src/main/java/com/example/photoconsumer/processamento/consumer/PubSubPushController.java` e `PhotoProcessingMessageHandler.java`
...
- [X] T096 [US6] Manter no consumer somente configurações pertencentes à aplicação e documentar o micro-retry finito do Storage em conjunto com a macro-redelivery configurada na subscription Push para evitar retry storm em `backend/photo-consumer/src/main/resources/application.yml`
...
- [X] T099 [US6] Implementar endpoint Push de `photo-consumer-dlq-sub` delegando ao handler existente sem criar segunda DLT em `backend/photo-consumer/src/main/java/com/example/photoconsumer/processamento/consumer/PubSubPushController.java`
